class Birthday
  ...
  def today?(day, month)
    date_to_year_day(day, month) == time_to_year_day(Time.now)
  end

  def days_until_bday(day, month)
    days = date_to_year_day(day, month) - time_to_year_day(Time.now)
    days = 365 - days.abs if days.negative?
    days == 1 ? "#{days} day" : "#{days} days"
  end

  def time_to_year_day(time)
    time.strftime('%j').to_i
  end

  def date_to_year_day(day, month)
    time_to_year_day(Time.parse("#{month}-#{day}"))
  end
end
